...
/**
 * Flying enemy. On detection it locks onto the Knight's vertical position, moves to that height and
 * performs a purely horizontal charge.
 */
public final class WingedSentry implements Damageable, Pogoable, Knockbackable {
    ...
    private static final float MIN_POSITIONING_DURATION = 0.30f;

    private static final float POSITION_TOLERANCE = 2f;

    private static final float CHARGE_ANTICIPATION_DURATION = 0.40f;

    private static final float CHARGE_SPEED = 470f;

    private static final float MAX_CHARGE_DURATION = 1.15f;

    private static final float RETURN_SPEED = 230f;

    private static final float HOME_TOLERANCE = 3f;

    private static final float ATTACK_COOLDOWN_DURATION = 0.65f;

    private static final float FLASH_DURATION = 0.12f;

    private static final float DEATH_GRAVITY = -950f;

    private static final float MAX_DEATH_FALL_SPEED = -700f;

    private static final float DEATH_LAND_DURATION = 0.40f;
    ...
    public WingedSentry(float spawnX, float spawnY, boolean initialFacingRight) {
        this.spawnX = spawnX;
        this.spawnY = spawnY;
    ...
    public void update(
        ...
        if (state == WingedSentryState.CORPSE) {
        ...
        if (state == WingedSentryState.DYING_AIR) {
            updateDeathAir(safeDelta, platformWorld);

            updateDetectionBounds();
            return;
        }

        if (state == WingedSentryState.DYING_LAND) {
        ...
        switch (state) {
            case IDLE_HOVERING -> updateIdle(playerBounds, playerAlive);

            case POSITIONING -> updatePositioning(safeDelta);

            case CHARGE_ANTICIPATE -> updateChargeAnticipation();

            case CHARGING -> updateCharge(safeDelta, platformWorld);

            case RETURNING_HOME -> updateReturningHome(safeDelta);

            case DYING_AIR, DYING_LAND, CORPSE -> {}
        }
    ...
    public void takeDamage(int damage) {
        ...
        if (health == 0) {
            ...
            state = WingedSentryState.DYING_AIR;
    ...
    public WingedSentryAnimationType getAnimationType() {
        return switch (state) {
            case IDLE_HOVERING -> WingedSentryAnimationType.IDLE;

            case POSITIONING, RETURNING_HOME -> WingedSentryAnimationType.TURN_TO_FLY;

            case CHARGE_ANTICIPATE -> WingedSentryAnimationType.CHARGE_ANTICIPATE;

            case CHARGING -> WingedSentryAnimationType.CHARGE;

            case DYING_AIR -> WingedSentryAnimationType.DEATH_AIR;

            case DYING_LAND, CORPSE -> WingedSentryAnimationType.DEATH_LAND;
        };
    }

    public boolean isAttackHeightLocked() {
        return state == WingedSentryState.POSITIONING
                || state == WingedSentryState.CHARGE_ANTICIPATE
                || state == WingedSentryState.CHARGING;
    }
    ...
    public boolean isCorpse() {
        return state == WingedSentryState.CORPSE;
    }

    public float getLockedChargeCenterY() {
        return lockedChargeY + BODY_HEIGHT / 2f;
    }
    ...
    private void updateIdle(Rectangle playerBounds, boolean playerAlive) {
        if (!playerAlive || playerBounds == null || detectionCooldownRemaining > 0f) {
            return;
        }
        ...
        lockAttackFromPlayer(playerBounds);

        changeState(WingedSentryState.POSITIONING);
    }

    private void lockAttackFromPlayer(Rectangle playerBounds) {
        ...
        chargeDirection = playerCenterX >= enemyCenterX ? 1 : -1;

        lockedChargeY = playerCenterY - BODY_HEIGHT / 2f;

        facingRight = chargeDirection > 0;
    }

    private void updatePositioning(float delta) {
        float difference = lockedChargeY - bounds.y;

        float maximumMovement = POSITIONING_SPEED * delta;

        if (Math.abs(difference) <= maximumMovement) {
            bounds.y = lockedChargeY;
        } else {
            bounds.y += Math.signum(difference) * maximumMovement;
        }

        boolean correctlyPositioned = Math.abs(lockedChargeY - bounds.y) <= POSITION_TOLERANCE;

        if (correctlyPositioned && stateTime >= MIN_POSITIONING_DURATION) {
            bounds.y = lockedChargeY;

            changeState(WingedSentryState.CHARGE_ANTICIPATE);
        }
    }
    ...
    private void updateChargeAnticipation() {
        ...
        if (stateTime < CHARGE_ANTICIPATION_DURATION) {
            return;
        }

        changeState(WingedSentryState.CHARGING);
    }
    ...
    private void updateCharge(float delta, PlatformWorld platformWorld) {
        ...
        if (stateTime >= MAX_CHARGE_DURATION || !canChargeForward(delta, platformWorld)) {
            changeState(WingedSentryState.RETURNING_HOME);
    ...
    private boolean canChargeForward(float delta, PlatformWorld platformWorld) {
        nextBounds.set(bounds);

        nextBounds.x += chargeDirection * CHARGE_SPEED * delta;

        if (nextBounds.x < 0f || nextBounds.x + nextBounds.width > platformWorld.getWorldWidth()) {
            return false;
        }

        return !platformWorld.overlapsSolid(nextBounds);
    }

    private void updateReturningHome(float delta) {
        float distanceX = spawnX - bounds.x;

        float distanceY = spawnY - bounds.y;
        ...
        float movement = RETURN_SPEED * delta;
        ...
        float normalizedX = distanceX / distance;

        float normalizedY = distanceY / distance;

        bounds.x += normalizedX * movement;

        bounds.y += normalizedY * movement;
    ...
    private void finishReturningHome() {
        ...
        chargeDirection = facingRight ? 1 : -1;

        detectionCooldownRemaining = ATTACK_COOLDOWN_DURATION;

        changeState(WingedSentryState.IDLE_HOVERING);
    }

    private void updateDeathAir(float delta, PlatformWorld platformWorld) {
        stateTime += delta;

        updateDeathSideFall(delta);

        float previousBottom = bounds.y;

        deathVerticalVelocity += DEATH_GRAVITY * delta;

        deathVerticalVelocity = Math.max(deathVerticalVelocity, MAX_DEATH_FALL_SPEED);

        float nextBottom = bounds.y + deathVerticalVelocity * delta;

        float landingSurface =
                platformWorld.findLandingSurfaceY(bounds, previousBottom, nextBottom);
        ...
        if (!Float.isNaN(landingSurface)) {
            ...
            changeState(WingedSentryState.DYING_LAND);
    ...
    private void updateDeathLand(float delta) {
        stateTime += delta;
    ...
    private void updateDetectionBounds() {
        if (!isAlive() || state != WingedSentryState.IDLE_HOVERING) {
            detectionBounds.set(0f, 0f, 0f, 0f);
    ...
    private void updateDetectionCooldown(float delta) {
        if (detectionCooldownRemaining <= 0f) {
            return;
        }
    ...
    private void changeState(WingedSentryState newState) {
        if (state == newState) {
            return;
        }
    ...
    private void respawn() {
        ...
        state = WingedSentryState.IDLE_HOVERING;
