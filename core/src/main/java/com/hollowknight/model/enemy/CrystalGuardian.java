...
/**
 * Stationary ranged enemy. Watches the area in front of itself, fires a continuous laser, performs
 * a temporary enraged charge and then returns to its original position.
 */
public final class CrystalGuardian implements Damageable, Pogoable, Knockbackable {
    ...
    public CrystalGuardian(float spawnX, float spawnY, boolean initialFacingRight) {
        this.spawnX = spawnX;
        this.spawnY = spawnY;
    ...
    public void update(
        ...
        if (state == CrystalGuardianState.CORPSE) {
        ...
        if (state == CrystalGuardianState.DYING) {
        ...
        switch (state) {
            case IDLE_WATCHING -> updateIdle(playerBounds, playerAlive);

            case SHOOTING -> updateShooting();

            case ENRAGED_CHARGING -> updateEnragedCharge(safeDelta, platformWorld);

            case RETURNING_HOME -> updateReturningHome(safeDelta, platformWorld);

            case TURNING_TO_PLAYER -> updateTurningToPlayer();

            case TURNING_HOME -> updateTurningHome();
    ...
    public void takeDamage(int damage) {
        ...
        if (health == 0) {
            ...
            state = CrystalGuardianState.DYING;
    ...
    public CrystalGuardianAnimationType getAnimationType() {
        return switch (state) {
            case IDLE_WATCHING -> CrystalGuardianAnimationType.IDLE;

            case SHOOTING -> CrystalGuardianAnimationType.SHOOT;

            case ENRAGED_CHARGING -> CrystalGuardianAnimationType.RUN;

            case RETURNING_HOME -> CrystalGuardianAnimationType.EVADE;

            case TURNING_TO_PLAYER, TURNING_HOME -> CrystalGuardianAnimationType.TURN;

            case DYING, CORPSE -> CrystalGuardianAnimationType.DEATH_LAND;
        };
    }
    ...
    public boolean isLaserActive() {
        return state == CrystalGuardianState.SHOOTING
    ...
    public boolean isCorpse() {
        return state == CrystalGuardianState.CORPSE;
    }
    ...
    private void updateIdle(Rectangle playerBounds, boolean playerAlive) {
        if (!playerAlive
                || playerBounds == null
                || !isPlayerWithinBidirectionalVision(playerBounds)) {
            return;
        }

        boolean playerIsToTheRight = isPlayerToTheRight(playerBounds);

        if (playerIsToTheRight != facingRight) {
            turnTargetFacingRight = playerIsToTheRight;

            changeState(CrystalGuardianState.TURNING_TO_PLAYER);
            return;
        }
    ...
    private boolean isPlayerWithinBidirectionalVision(Rectangle playerBounds) {
        float visionBottom = bounds.y + VISION_Y_OFFSET;

        float visionTop = visionBottom + VISION_HEIGHT;

        boolean overlapsVertically =
                playerBounds.y < visionTop && playerBounds.y + playerBounds.height > visionBottom;

        if (!overlapsVertically) {
            return false;
        }

        float detectionLeft = bounds.x - VISION_WIDTH;

        float detectionRight = bounds.x + bounds.width + VISION_WIDTH;

        return playerBounds.x < detectionRight
                && playerBounds.x + playerBounds.width > detectionLeft;
    }

    private boolean isPlayerToTheRight(Rectangle playerBounds) {
        float guardianCenterX = bounds.x + bounds.width / 2f;
    ...
    private void updateShooting() {
        ...
        changeState(CrystalGuardianState.ENRAGED_CHARGING);
    }
    ...
    private void updateEnragedCharge(float delta, PlatformWorld platformWorld) {
        if (stateTime >= ENRAGED_DURATION) {
            beginReturningHome();
            return;
        }

        if (!canMoveForward(attackDirection, ENRAGED_SPEED, delta, platformWorld)) {
            beginReturningHome();
            return;
        }

        bounds.x += attackDirection * ENRAGED_SPEED * delta;
    }

    private void beginReturningHome() {
        clearLaser();

        changeState(CrystalGuardianState.RETURNING_HOME);
    }

    private void updateReturningHome(float delta, PlatformWorld platformWorld) {
        float distanceToHome = spawnX - bounds.x;

        if (Math.abs(distanceToHome) <= HOME_TOLERANCE) {
            finishReturningHome();
            return;
        }

        int returnDirection = distanceToHome > 0f ? 1 : -1;

        facingRight = returnDirection > 0;

        float movement = returnDirection * RETURN_SPEED * delta;

        if (Math.abs(movement) >= Math.abs(distanceToHome)) {
            bounds.x = spawnX;
            finishReturningHome();
            return;
        }

        if (!canMoveForward(returnDirection, RETURN_SPEED, delta, platformWorld)) {
    ...
    private void finishReturningHome() {
        ...
        if (facingRight != initialFacingRight) {
            changeState(CrystalGuardianState.TURNING_HOME);
        } else {
            changeState(CrystalGuardianState.IDLE_WATCHING);
        }
    }
    ...
    private void updateTurningToPlayer() {
        ...
        facingRight = turnTargetFacingRight;
    ...
    private void updateDeath(float delta) {
        ...
        state = CrystalGuardianState.CORPSE;
    ...
    private void updateVisionBounds() {
        if (state != CrystalGuardianState.IDLE_WATCHING || !isAlive()) {
    ...
    private void updateLaserBounds(PlatformWorld platformWorld) {
        if (!isLaserActive()) {
            clearLaser();
            return;
        }

        float laserY = bounds.y + LASER_Y_OFFSET;

        float laserStartX;

        if (attackDirection > 0) {
            laserStartX = bounds.x + bounds.width + LASER_START_GAP;
        } else {
            laserStartX = bounds.x - LASER_START_GAP;
        }

        float laserLength =
                platformWorld.getHorizontalRayLength(
                        laserStartX, laserY, LASER_HEIGHT, attackDirection, LASER_MAX_RANGE);

        if (attackDirection > 0) {
            laserBounds.set(laserStartX, laserY, laserLength, LASER_HEIGHT);
        } else {
            laserBounds.set(laserStartX - laserLength, laserY, laserLength, LASER_HEIGHT);
        }
    }

    private void clearLaser() {
        laserBounds.set(0f, 0f, 0f, 0f);
    }
    ...
    private void changeState(CrystalGuardianState newState) {
        if (state == newState) {
            return;
        }
    ...
    private void respawn() {
        ...
        turnTargetFacingRight = facingRight;

        state = CrystalGuardianState.IDLE_WATCHING;
