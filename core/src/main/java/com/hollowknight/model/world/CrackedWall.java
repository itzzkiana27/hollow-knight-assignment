...
/**
 * Runtime state for a breakable wall described by a Tiled object.
 */
public final class CrackedWall {
    ...
    public CrackedWall(
        ...
        this.brokenAppearanceHit = Math.max(
            1,
            brokenAppearanceHit
        );
        this.destroyHit = Math.max(
            this.brokenAppearanceHit,
            destroyHit
        );
    ...
    public void update(float delta) {
        ...
        shakeTimeRemaining = Math.max(
            0f,
            shakeTimeRemaining - safeDelta
        );

        for (
            int index = debrisPieces.size - 1;
            index >= 0;
            index--
        ) {
            DebrisPiece piece =
                debrisPieces.get(index);
