...
public class GameSettings {
    ...
    private static final String CONTROLS_VERSION_KEY = "controlsVersion";

    private static final int CURRENT_CONTROLS_VERSION = 3;

    private static final String SLIDER_DEFAULTS_VERSION_KEY = "sliderDefaultsVersion";
    ...
    public static final float DEFAULT_BRIGHTNESS = (MIN_BRIGHTNESS + MAX_BRIGHTNESS) / 2f;
    ...
    public void save() {
        ...
        preferences.putFloat("soundEffectsVolume", soundEffectsVolume);

        preferences.putFloat("brightness", brightness);

        preferences.putString("musicStyle", musicStyle);

        preferences.putString("language", language);

        preferences.putString("menuTheme", menuTheme);

        preferences.putInteger("moveLeftKey", moveLeftKey);

        preferences.putInteger("moveRightKey", moveRightKey);

        preferences.putInteger("jumpKey", jumpKey);

        preferences.putInteger("dashKey", dashKey);

        preferences.putInteger("attackKey", attackKey);

        preferences.putInteger("upKey", upKey);

        preferences.putInteger("downKey", downKey);

        preferences.putInteger("alternateAttackKey", alternateAttackKey);

        preferences.putInteger("focusKey", focusKey);

        preferences.putInteger("fireballKey", fireballKey);

        preferences.putInteger("screamKey", screamKey);

        preferences.putInteger("interactKey", interactKey);

        preferences.putInteger("dialogueAdvanceKey", dialogueAdvanceKey);

        preferences.putInteger("inventoryKey", inventoryKey);

        preferences.putInteger("pauseKey", pauseKey);

        preferences.putInteger(CONTROLS_VERSION_KEY, CURRENT_CONTROLS_VERSION);
    ...
    public void setBrightness(float brightness) {
        this.brightness = MathUtils.clamp(brightness, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
    }
    ...
    private void load() {
        ...
        musicVolume = preferences.getFloat("musicVolume", DEFAULT_MUSIC_VOLUME);

        soundEffectsVolume =
                preferences.getFloat("soundEffectsVolume", DEFAULT_SOUND_EFFECTS_VOLUME);

        brightness = preferences.getFloat("brightness", DEFAULT_BRIGHTNESS);

        musicStyle = preferences.getString("musicStyle", "Original");

        language = preferences.getString("language", "en");

        menuTheme = preferences.getString("menuTheme", "voidheart");

        int controlsVersion = preferences.getInteger(CONTROLS_VERSION_KEY, 0);
    ...
    private void loadPrimaryControls() {
        moveLeftKey = preferences.getInteger("moveLeftKey", KeyBindings.DEFAULT_MOVE_LEFT);

        moveRightKey = preferences.getInteger("moveRightKey", KeyBindings.DEFAULT_MOVE_RIGHT);

        jumpKey = preferences.getInteger("jumpKey", KeyBindings.DEFAULT_JUMP);

        dashKey = preferences.getInteger("dashKey", KeyBindings.DEFAULT_DASH);

        attackKey = preferences.getInteger("attackKey", KeyBindings.DEFAULT_ATTACK);
    }

    private void loadAdditionalControls() {
        upKey = preferences.getInteger("upKey", KeyBindings.DEFAULT_UP);

        downKey = preferences.getInteger("downKey", KeyBindings.DEFAULT_DOWN);

        alternateAttackKey =
                preferences.getInteger("alternateAttackKey", KeyBindings.DEFAULT_ALTERNATE_ATTACK);

        focusKey = preferences.getInteger("focusKey", KeyBindings.DEFAULT_FOCUS);

        fireballKey = preferences.getInteger("fireballKey", KeyBindings.DEFAULT_FIREBALL);

        screamKey = preferences.getInteger("screamKey", KeyBindings.DEFAULT_SCREAM);

        interactKey = preferences.getInteger("interactKey", KeyBindings.DEFAULT_INTERACT);

        dialogueAdvanceKey =
                preferences.getInteger("dialogueAdvanceKey", KeyBindings.DEFAULT_DIALOGUE_ADVANCE);

        inventoryKey = preferences.getInteger("inventoryKey", KeyBindings.DEFAULT_INVENTORY);

        pauseKey = preferences.getInteger("pauseKey", KeyBindings.DEFAULT_PAUSE);
    }

    private void migrateLegacySliderDefaults() {
        int defaultsVersion = preferences.getInteger(SLIDER_DEFAULTS_VERSION_KEY, 0);

        if (defaultsVersion >= CURRENT_SLIDER_DEFAULTS_VERSION) {
            return;
        }

        if (!preferences.contains("musicVolume") || isApproximately(musicVolume, 0.8f)) {
            musicVolume = DEFAULT_MUSIC_VOLUME;
        }
        ...
        if (!preferences.contains("brightness") || isApproximately(brightness, 1f)) {
            brightness = DEFAULT_BRIGHTNESS;
        }

        preferences.putInteger(SLIDER_DEFAULTS_VERSION_KEY, CURRENT_SLIDER_DEFAULTS_VERSION);

        save();
    }

    private boolean isApproximately(float first, float second) {
    ...
    private void setDefaultControls() {
        ...
        alternateAttackKey = KeyBindings.DEFAULT_ALTERNATE_ATTACK;
        ...
        dialogueAdvanceKey = KeyBindings.DEFAULT_DIALOGUE_ADVANCE;
