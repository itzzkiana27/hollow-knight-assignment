...
public final class CrackedWallRenderer {
    ...
    public CrackedWallRenderer(CrackedWall wall) {
        ...
        initialTexture = loadTexture(wall.getInitialSpritePath());

        brokenTexture = loadTexture(wall.getBrokenSpritePath());

        debrisTextureA = loadTexture(wall.getDebrisSpriteAPath());

        debrisTextureB = loadTexture(wall.getDebrisSpriteBPath());
    }

    public void draw(SpriteBatch batch, CrackedWall wall) {
        if (wall == null) {
            return;
        }

        if (!wall.isDestroyed()) {
            Texture wallTexture = wall.usesBrokenAppearance() ? brokenTexture : initialTexture;

            if (wallTexture != null) {
                batch.draw(
                        wallTexture,
                        wall.getBounds().x + wall.getShakeOffsetX(),
                        wall.getBounds().y,
        ...
        for (CrackedWall.DebrisPiece piece : wall.getDebrisPieces()) {
            Texture texture = piece.getSpriteIndex() == 0 ? debrisTextureA : debrisTextureB;
